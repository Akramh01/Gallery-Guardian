...
$request_url = curl_init("http://192.168.4.1:8080/json.htm?type=devices&filter=all&used=true&order=Name&param=getdevices&rid=31");
...
if ($data['status']=='OK') {
    ...
    if (isset($data['result'])) {
        ...
        $luminausite = $data['result'][0]['Data'];
        ...
        if($verif->rowCount()==0){
            ...
            $request = $bdd->prepare("INSERT INTO Capteur (idC, etatC,illuminance, type, nom, date, heure)
            VALUES (:id, :etat, :luminausite, :type, :name, :date, :heure)");
            $request->execute([
                'id' => $id,
                'etat' => $etat,
                'luminausite' => $luminausite,
                'type' => $type,
                'name' => $name,
                'date' => $date,
                'heure' => $heure
            ...
            $request = $bdd->prepare("UPDATE Capteur SET illuminance = :luminausite, date = :date, heure = :heure WHERE idC = :id");
            $request->execute([
                'luminausite' => $luminausite,
                'date' => $date,
                'heure' => $heure,
